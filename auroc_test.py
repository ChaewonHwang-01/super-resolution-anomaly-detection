...
def plot_roc_curve_and_get_auroc(scores: List[float], labels: List[int], save_path: str) -> float:
    ...
    if roc_auc_score is not None:
        ...
        label_auc = float(auc(fpr, tpr))
    else:
        auroc = compute_auroc_fallback(scores_np, labels_np)
        fpr, tpr = roc_curve_fallback(scores_np, labels_np)
        label_auc = float(np.trapz(tpr, fpr))

    plt.figure(figsize=(6, 6))
    plt.plot(fpr, tpr, label=f"AUROC = {label_auc:.4f}")
...
def main():
    ...
    print("\n=== Step 1: recursive scan + histogram + amap + score + dilated overlay (+ AUROC) ===")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=== Pixel-diff Histogram + Dilated Threshold Overlay + Image Score Results (+ AUROC) ===\n")
