...
class SRCNNx2(nn.Module):
    ...
    """
    def __init__(self, scale: int = 2, in_channels: int = 1, fn: int = 32, dfn: int = 64):
        super().__init__()
        self.scale = scale

        self.first_part = nn.Conv2d(in_channels, dfn, kernel_size=9, stride=1, padding=9 // 2)
        self.mid_part   = nn.Conv2d(dfn, fn,  kernel_size=5, stride=1, padding=5 // 2)
        self.last_part  = nn.Conv2d(fn,  in_channels, kernel_size=5, stride=1, padding=5 // 2)

        self.relu = nn.ReLU(inplace=True)

        # same init as your original code (kept for completeness)
...
class AE_512_32(nn.Module):
    ...
    """
    def __init__(self, in_channels: int = 1, base_channels: int = 64, max_channels: int = 256):
        super().__init__()
...
class SR2_AE(nn.Module):
    ...
    Updated:
      AE compresses down to 32x32 feature map.
    """
    def __init__(
        self,
        in_channels: int = 1,
